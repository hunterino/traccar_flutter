...
class ShortcutActivity : AppCompatActivity() {
    ...
    private fun sendAlarm() {
        PositionProviderFactory.create(this, object : PositionProvider.PositionListener {
            override fun onPositionUpdate(position: Position) {
                val preferences = PreferenceManager.getDefaultSharedPreferences(this@ShortcutActivity)
                val request = formatRequest(preferences.getString(TraccarController.KEY_URL, null)!!, position, ALARM_SOS)
    ...
    private fun executeAction(intent: Intent): Boolean {
        ...
        if (action != null) {
            when (action) {
                ACTION_START -> {
                    PreferenceManager.getDefaultSharedPreferences(this).edit().putBoolean(TraccarController.KEY_STATUS, true).apply()
                    ContextCompat.startForegroundService(this, Intent(this, TrackingService::class.java))
                    Toast.makeText(this, R.string.status_service_create, Toast.LENGTH_SHORT).show()
                }
                ACTION_STOP -> {
                    PreferenceManager.getDefaultSharedPreferences(this).edit().putBoolean(TraccarController.KEY_STATUS, false).apply()
