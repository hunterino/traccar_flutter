...
class PositionProvider: NSObject, CLLocationManagerDelegate {
    ...
    override init() {
        ...
        interval = userDefaults.double(forKey: PreferenceKeys.frequency.rawValue)
        distance = userDefaults.double(forKey: PreferenceKeys.distance.rawValue)
        angle = userDefaults.double(forKey: PreferenceKeys.angle.rawValue)
        ...
        switch userDefaults.string(forKey: PreferenceKeys.accuracyLevel.rawValue) ?? "medium" {
        case "high":
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let location = locations.last {
            if lastLocation == nil
                || location.timestamp.timeIntervalSince(lastLocation!.timestamp) >= interval/1000
